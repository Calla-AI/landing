  ...
  return (
    <div className="max-w-4xl mx-auto p-6 rounded-lg">
      <div className="flex items-start">
          ...
          <p className="text-primary">
            {text.split(' ').map((word, index) => {
              if (positiveWords.includes(word)) {
                ...
                return <span key={index} className="font-semibold text-red-500">{word}{' '}</span>;
              } else {
        ...
        <div className="ml-4">
          <Badge className={`text-${sentimentResult === 'Positive' ? 'green' : sentimentResult === 'Negative' ? 'red' : 'gray'}-500`}>
            {sentimentResult}
          </Badge>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 rounded-lg">
      <div className="flex items-start">
        ...
        <div className="ml-4">
          <Badge className={`text-${result === 'Technical Support' ? 'green' : result === 'Sales' ? 'Purple' : result === 'Billing' ? 'blue' : 'gray'}-500`}>
            {result}
          </Badge>
        </div>
      </div>
